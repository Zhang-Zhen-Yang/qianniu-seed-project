        ...
        return (
            <View style={styles.listItem} onClick={()=>{this.props.actionPress( )}}>                
                ...
                <View style={{alginItems:'center',flex:1,paddingLeft:10,paddingRight:10}}>                    
                    <Text text-overflow='ellipsis' style={{flex:1,fontSize:'30rem',lines:2,textOverflow:'ellipsis',color:'#666666'}}>{this.props.item.title}</Text>                     
                    <Text style={{fontSize:'35rem',color:'orange',fontWeight:'bold'}}>{this.props.item.price}</Text>
                </View>                
            </View>
        );
    }
